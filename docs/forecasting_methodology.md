...
**Step 3 experimental record:** The first learned test used one horizon-conditioned direct `HistGradientBoostingRegressor` per fold, globally pooled across the 70 series. It used origin-anchored lags 1/7/14/28/56, rolling mean and population standard deviation over 7/28/56 days, forecast horizon, store/department/state identity, and known target-date weekday/day/month/year/trend/event/event-type/state-SNAP context. Prices were excluded. Training origins were sampled weekly with every 28-day label window contained in fold training. The fixed untuned model achieved development macro RMSSE 0.77916 versus 1.06690 for historical mean and beat both main baselines in all six folds. This is a promising experimental result, not a freeze of the algorithm, features, or final model family. Full results are in `reports/forecasting/first_global_model_results.md`; the lockbox remains untouched.
